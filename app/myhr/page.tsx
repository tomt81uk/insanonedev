...
type Workflow = {
  id: number;
  kind: "bank_change" | "address_change" | "leave_request" | "onboarding" | "rtw_check";
  employee_id: number;
  status: "pending_hr" | "pending_manager" | "pending_employee" | "approved" | "rejected";
...
async function getData() {
  ...
  const payslips: Payslip[] = [
    { id: 7001, employee_id: 101, period: "2025-08", gross: 42000, net: 40000, currency: "AED", pay_date: "2025-08-29" },
    { id: 7002, employee_id: 102, period: "2025-08", gross: 26000, net: 24500, currency: "AED", pay_date: "2025-08-29" },
    { id: 7003, employee_id: 103, period: "2025-08", gross: 22000, net: 21000, currency: "AED", pay_date: "2025-08-29" },
    { id: 7004, employee_id: 104, period: "2025-08", gross: 32000, net: 30500, currency: "AED", pay_date: "2025-08-29" },
    // A prior month to show trend
    { id: 7101, employee_id: 101, period: "2025-07", gross: 42000, net: 40000, currency: "AED", pay_date: "2025-07-31" },
    { id: 7102, employee_id: 102, period: "2025-07", gross: 26000, net: 24500, currency: "AED", pay_date: "2025-07-31" },
    { id: 7103, employee_id: 103, period: "2025-07", gross: 22000, net: 21000, currency: "AED", pay_date: "2025-07-31" },
    { id: 7104, employee_id: 104, period: "2025-07", gross: 32000, net: 30500, currency: "AED", pay_date: "2025-07-31" },
  ];

  // Governance workflows (e.g., bank change approvals)
  const workflows: Workflow[] = [
    { id: 5001, kind: "bank_change", employee_id: 101, status: "pending_hr", created_at: "2025-09-10T10:00:00Z" },
    { id: 5002, kind: "address_change", employee_id: 103, status: "pending_manager", created_at: "2025-09-09T09:40:00Z" },
    { id: 5003, kind: "leave_request", employee_id: 102, status: "pending_manager", created_at: "2025-09-07T08:10:00Z" },
    { id: 5004, kind: "rtw_check", employee_id: 104, status: "approved", created_at: "2025-08-30T12:00:00Z" },
    { id: 5005, kind: "onboarding", employee_id: 102, status: "pending_employee", created_at: "2025-08-05T09:00:00Z" },
  ];
...
export default async function HRPage() {
  ...
  const now = new Date(); // Europe/London user timezone, but we'll compute UTC month
  ...
  const startersThisMonth = employees.filter((e) => withinMonth(e.hire_date, y, m));
  const leaversThisMonth = employees.filter((e) => e.end_date && withinMonth(e.end_date, y, m));

  // Absence approximation (count of approved leave days overlapping this month)
  const approvedLeave = leave_requests.filter((lr) => lr.approval_status === "approved");
  const absenceRecordsThisMonth = approvedLeave.filter((lr) => {
    ...
    return s.getUTCFullYear() <= y &&
      e.getUTCFullYear() >= y &&
      (s.getUTCMonth() <= m && e.getUTCMonth() >= m);
  });
  const absenceRatePct = headcount > 0 ? Math.min(100, (absenceRecordsThisMonth.length / headcount) * 100) : 0;

  // Payroll cost: sum latest period’s net by employee (for the latest month found)
  const periods = Array.from(new Set(payslips.map((p) => p.period))).sort().reverse();
  ...
  const inProgress = workflows.filter((w) => ["pending_manager", "pending_employee"].includes(w.status)); // amber
  ...
  const upcomingLeave = leave_requests
    .filter((lr) => new Date(lr.start_date + "T00:00:00Z") >= now && new Date(lr.start_date + "T00:00:00Z") <= in14)
      ...
      <main className="flex-1">
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-gray-500">
                KPIs for {thisPeriod} · {headcount} active workers · {fteTotal.toFixed(1)} FTE
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <a href="/reporting" className="rounded-xl border px-4 py-2 hover:bg-gray-50">Open Reporting</a>
              <a href="/admin" className="rounded-xl border px-4 py-2 hover:bg-gray-50">Admin Setup</a>
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <KpiCard label="Headcount" value={headcount.toString()} hint={`${fteTotal.toFixed(1)} FTE`} />
            <KpiCard label="Starters (month)" value={startersThisMonth.length.toString()} hint={startersThisMonth.map(s => s.name).slice(0,2).join(", ") || "—"} />
            <KpiCard label="Leavers (month)" value={leaversThisMonth.length.toString()} hint={leaversThisMonth.map(s => s.name).slice(0,2).join(", ") || "—"} />
            <KpiCard label="Monthly Net Pay" value={`AED ${monthlyNetAED.toLocaleString()}`} hint={`Period ${latestPeriod}`} />
          </div>
        </section>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {upcomingLeave.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="py-4 text-gray-500">No upcoming leave</td>
                    </tr>
                  ) : (
            ...
            <TableCard title={`Latest Payslips (${latestPeriod})`}>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {payslipsLatest.sort((a,b)=> b.net - a.net).slice(0,6).map((p) => (
                  ...
                  {workflows
                    ...
                    .map((w) => (
                      <tr key={w.id} className="border-t">
                        ...
                        <td className="py-2 capitalize">{w.status.replace("_", " ")}</td>
                        <td className="py-2">{w.created_at.substring(0,10)}</td>
                      </tr>
                    ))}
                  {workflows.filter((w) => !["approved", "rejected"].includes(w.status)).length === 0 && (
                    <tr>
                      <td colSpan={3} className="py-4 text-gray-500">No open workflows</td>
                    </tr>
                  )}
                </tbody>
              </table>
  ...
  return (
    <div className="rounded-2xl border p-4">
      <div className="flex items-center justify-between">
        ...
        <span className={`inline-flex h-3 w-3 rounded-full ${dotClass}`} aria-hidden />
      </div>
        ...
        {items.length === 0 ? (
          ...
          items.slice(0, 6).map((it, i) => (
            <li key={i} className="flex items-center justify-between">
              ...
              <a href={it.href} className="underline underline-offset-2">{it.action}</a>
            </li>
          ))
        )}
      </ul>
