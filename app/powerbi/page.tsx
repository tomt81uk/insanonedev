...
const currency = (n: number) =>
  n.toLocaleString(undefined, { style: "currency", currency: "AED", maximumFractionDigits: 0 });
...
const ALL_DEPTS: Dept[] = ["Engineering", "Operations", "Sales", "Finance", "HR"];

// Seed “realistic-ish” metrics for the last 12 months
function makeData() {
  const months = last12Months();
  const baseHeadcount = 120;
  const headcount = months.map((_, i) => Math.round(baseHeadcount + i * 2 - (i % 3 === 0 ? 3 : 0)));
  const pay = months.map((_, i) =>  baseHeadcount * 10000 + i * 150000 + (i % 4 ? 80000 : -40000));
  const turnover = months.map((_, i) => 1.2 + ((i % 5) - 2) * 0.25 + (i % 3 === 0 ? 0.4 : 0)); // %
  const absence = months.map((_, i) => 2.5 + ((i % 4) - 2) * 0.4 + (i % 6 === 0 ? 0.8 : 0)); // %
...
export default function DashboardBI() {
  const { months, headcount, pay, turnover, absence, split } = useMemo(makeData, []);
  const [yearFilter, setYearFilter] = useState<"All" | string>("All"); // "2025" etc. (labels already contain year)
  ...
  const hcByDeptAll = (val: number) =>
    ALL_DEPTS.map((d) => ({ dept: d, value: Math.round(val * split[d]) }));
  const hcCurrentDepts = hcByDeptAll(kpi.headcount);
  const hcByDept = deptFilter === "All"
    ? hcCurrentDepts
    : hcCurrentDepts.filter((d) => deptFilter.includes(d.dept));

  const yearsAvailable = Array.from(new Set(months.map((m) => m.label.split(" ")[1])));
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="flex items-end justify-between gap-4 flex-wrap">
            <div className="space-y-1">
              <h1 className="text-4xl sm:text-5xl font-semibold tracking-tight">Workforce Analytics</h1>
              <p className="text-gray-600">Power BI–style view of key people metrics.</p>
            </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                <button
                  ...
                  className={[
                    ...
                    deptFilter === "All" ? "border-[#2f6fed] text-[#2f6fed] bg-[#2f6fed]/5" : "border-gray-200"
                  ].join(" ")}
                >
                  All
                </button>
                {ALL_DEPTS.map((d) => {
                  const active = deptFilter !== "All" && deptFilter.includes(d);
                  return (
                    <button
                      type="button"
                      key={d}
                      className={[
                        "rounded-full border px-2 py-0.5 text-xs",
                        active ? "border-[#2f6fed] text-[#2f6fed] bg-[#2f6fed]/5" : "border-gray-200"
                      ].join(" ")}
                      onClick={() =>
                        setDeptFilter((prev) => {
          ...
          <section className="mt-6 grid gap-4 xl:grid-cols-3">
            {/* Line trends */}
            <Card title="Headcount & Pay (last 12 months)" subtitle="Line trend">
              <LineChart
            ...
            <Card title="Headcount by Department" subtitle="Current split">
              <Donut
                data={hcByDept.map((d) => ({ label: d.dept, value: d.value }))}
          ...
          <section className="mt-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold tracking-tight">Recent workforce changes</h2>
              <Link href="/actions" className="text-sm text-[#2f6fed] hover:underline">Open Actions</Link>
            </div>
            ...
            <div className="mt-3 overflow-hidden rounded-xl border border-gray-200">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-200">
                  {[
                    { when: "Today 09:14", emp: "A. Rahman", change: "New hire (Software Eng.)", dept: "Engineering", status: "Completed" },
                    { when: "Yesterday 16:02", emp: "L. Martins", change: "Salary update", dept: "Finance", status: "Approved" },
                    { when: "Mon 12:21", emp: "S. Khan", change: "Termination", dept: "Operations", status: "Pending" },
                    { when: "Sun 10:05", emp: "J. Patel", change: "Transfer to Sales", dept: "Sales", status: "Completed" },
                  ].map((r, i) => (
                      ...
                      <td className="px-4 py-2">
                        <span className="inline-flex items-center gap-2">
                          <span className={[
                            ...
                            r.status === "Completed" ? "bg-emerald-500" : r.status === "Approved" ? "bg-amber-500" : "bg-rose-500",
                          ].join(" ")} />
                          {r.status}
                        </span>
                      </td>
  ...
  return (
    <section className="rounded-2xl border border-gray-200 p-5 bg-white">
      <div className="mb-4">
        ...
        {props.subtitle && <div className="text-xs text-gray-600">{props.subtitle}</div>}
      </div>
      {props.children}
    </section>
  );
    ...
    <div className="rounded-2xl border border-gray-200 p-5 bg-white">
      ...
      <div className="mt-1 flex items-center gap-1 text-xs">
        ...
        <span className={up ? "text-emerald-700" : "text-rose-700"}>
          {label.includes("%") ? `${delta.toFixed(1)} pts` : (label.includes("Pay") ? currency(delta) : `${delta}`)}
        </span>
        {hint && <span className="text-gray-500">vs prev • {hint}</span>}
      </div>
    </div>
  );
}
...
function LineChart({
  ...
  // Normalize each series to 0..1 and plot; separate scales per series for clarity (BI style overlay)
  const W = 800, H = 220, PADL = 30, PADR = 10, PADT = 10, PADB = 24;
    ...
    <div className="w-full overflow-hidden">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-56">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((p, i) => (
          <line key={i} x1={PADL} x2={W - PADR} y1={PADT + p * (H - PADT - PADB)} y2={PADT + p * (H - PADT - PADB)} className="stroke-gray-200" strokeWidth="1" />
        ))}
        {/* Lines */}
        {paths.map((p, i) => (
          <path key={i} d={p.d} fill="none" stroke={i ? "#335784" : "#2f6fed"} strokeWidth="2.5" />
        ))}
        {/* X labels (sparse) */}
        {labels.map((l, i) =>
          i % Math.ceil(labels.length / 6) === 0 ? (
            <text key={i} x={X(i)} y={H - 6} textAnchor="middle" className="fill-gray-500 text-[10px]">
              {l}
            </text>
          ) : null
        )}
      ...
      <div className="mt-2 flex items-center gap-3 text-xs text-gray-600">
        <LegendDot color="#2f6fed" /> {series[0]?.name}
        <LegendDot color="#335784" /> {series[1]?.name}
      </div>
    </div>
  );
}
...
function BarsTwo({
  ...
  const X = (i: number) => PADL + (i + 0.5) * ((W - PADL - PADR) / groups);
  ...
  return (
    <div className="w-full overflow-hidden">
      ...
      <div className="mt-2 flex items-center gap-3 text-xs text-gray-600">
        <LegendDot color="#2f6fed" /> {a.name}
        <LegendDot color="#335784" /> {b.name}
      </div>
    </div>
  );
}
...
function Donut({
  ...
  let acc = 0;
  const colors = ["#2f6fed", "#335784", "#7a94bf", "#9fb3d7", "#c7d6ee"];
    ...
    <div className="flex items-center gap-6">
      <svg viewBox="0 0 180 180" className="h-40 w-40">
        <g transform="translate(90,90)">
          ...
          <text textAnchor="middle" dominantBaseline="middle" className="fill-[#0a1a3a] text-[18px] font-semibold">
            {totalLabel ?? total}
          </text>
        </g>
      </svg>
      <div className="space-y-2">
        {data.map((d, i) => (
          <div key={i} className="flex items-center gap-2 text-sm">
            ...
            <span className="tabular-nums text-gray-700">{d.value}</span>
          </div>
        ))}
